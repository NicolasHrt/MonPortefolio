export function loadYoutubeVideo() {
    ...
    const LatestCoverContainer = document.querySelector("#Latest-Cover-Container");
    console.log(LatestCoverContainer);
    const LatestCompositionContainer = document.querySelector("#Latest-Composition-Container");

    // Fetch the latest cover video
    fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&order=date&maxResults=1&playlistId=${coverPlaylistId}&key=${APIkey}`)
    ...
    fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&order=date&maxResults=1&playlistId=${compositionPlaylistId}&key=${APIkey}`)
